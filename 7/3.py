...
def load_data(path, feature_num=2):
    ...
    for line in f.readlines():
        ...
        data_tmp = []
        if len(lines) != feature_num:  # 判断特征的维数是否正确
            continue
...
def gaussian_kernel(distance, bandwidth):
    ...
    right = np.asmatrix(np.zeros((m, 1)))  # 声明mx1的矩阵
    for i in range(m):
...
def shift_point(point, points, kernel_bandwidth):
    ...
    input:   point(mat)需要计算的点
            points(array)所有的样本点
            kernel_bandwidth(int)核函数的带宽s
    output: point_shifted(mat)漂移后的点
    """
    points = np.asmatrix(points)
    m = np.shape(points)[0]  # 样本的个数
    # 计算距离
    point_distances = np.asmatrix(np.zeros((m, 1)))
    for i in range(m):
...
def train_mean_shift(points, kenel_bandwidth=2):
    ...
    output: points(mat):特征点
            ...
            group(array):类别
    """
    mean_shift_points = np.asmatrix(points)  # 需要聚类的样本点
    max_min_dist = 1
    iteration = 0  # 训练的代数
    ...
    return np.asmatrix(points), mean_shift_points, group
...
if __name__ == "__main__":
    ...
    # 导入数据集
    print("----------1.load data ------------")
    data = load_data("7/data.txt", 2)
    N = len(data)
    # 训练，h=2
    print("----------2.training ------------")
    points, shift_points, cluster = train_mean_shift(data, 2)
    # 保存所属的类别文件
    save_result("7/center.txt", shift_points)
    data = np.array(data)
    for i in range(N):
        if cluster[i] == 0:
