...
class Net(torch.nn.Module):
    def __init__(self):
        ...
        # 第一个卷积层，输入通道1，输出通道10，卷积核5x5
        self.conv1 = torch.nn.Conv2d(1, 10, kernel_size=5)
        # 第二个卷积层，输入通道10，输出通道20，卷积核5x5
        self.conv2 = torch.nn.Conv2d(10, 20, kernel_size=5)
        # 全连接层，输入维度20*4*4=320，输出维度10
        self.fc = torch.nn.Linear(320, 10)

    def forward(self, x):
        # 第一个卷积层 + ReLU + 最大池化
        x = F.relu(self.conv1(x))  # 输出尺寸: (batch, 10, 24, 24)
        x = F.max_pool2d(x, 2)  # 输出尺寸: (batch, 10, 12, 12)

        # 第二个卷积层 + ReLU + 最大池化
        x = F.relu(self.conv2(x))  # 输出尺寸: (batch, 20, 8, 8)
        x = F.max_pool2d(x, 2)  # 输出尺寸: (batch, 20, 4, 4)

        # 展平张量，准备输入全连接层
        x = x.view(-1, 320)  # 展平为: (batch, 320)
...
optimizer = torch.optim.SGD(
    model.parameters(), lr=learning_rate, momentum=momentum, weight_decay=0.01
)  # SGD optimizer
